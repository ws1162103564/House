...
<META name=GENERATOR content="MSHTML 8.00.7601.17514">
  ...
  <script language="JavaScript">
      $(function () {
          //1.发送异步请求加载类型数据
          $.post("getAlltypexiangmu.do",null,function(data){
              //循环一次一行对就一个选项
      ...
      $(function () {
          //1.发送异步请求加载类型数据
          $.post("getDistrictall.do",null,function(data){
              //循环一次一行对就一个选项
              for (var i=0;i<data.length;i++){
                  //创建dom节点
                  var option=$("<option value="+data[i].id+">"+data[i].name+"</option>");
      ...
      function changeStreet(){
          ...
          //加载数据
          $.post("getStreetByDid.do",{"did":did},function(data){
              //循环一次一行对就一个选项
              for (var i=0;i<data.length;i++){
                  //创建dom节点
                  var option=$("<option value="+data[i].id+">"+data[i].name+"</option>");
...
<DL class="search clearfix">
  <FORM id=sform method=post action=/page/searchHouse.do>
    <input type="hidden" value="1"  id="setPage" name="page">
    标题:<input type="text" value="${HouseCondition.title}" name="title">
    ...
    价格:<input type="text" name="startPrice" value="${HouseCondition.startPrice}">-<input type="text" value="${HouseCondition.endPrice}" name="endPrice">
